...
# %% auto 0
__all__ = ['normalize_dataframe', 'plot_mds_columns', 'plot_pca_columns', 'intensity_filter', 'top_n_rows_driving_columns']
...
def intensity_filter(indf, top=500):
    '''
    return top n rows from df 
    based on mean average of rows
    '''
    indf['mean']=indf.mean(axis=1,skipna=True)
    indf=indf.sort_values('mean',ascending=False).head(top)  
    del indf['mean']
    return indf

# %% ../nbs/01_dim_red.ipynb 10
